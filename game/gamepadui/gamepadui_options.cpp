...
void OnResolutionsNeedUpdate(IConVar *var, const char *pOldValue,
                             float flOldValue);
...
ConVar _gamepadui_aspectratio("_gamepadui_aspectratio", "0", FCVAR_NONE, "",
                              OnResolutionsNeedUpdate);
ConVar _gamepadui_displaymode("_gamepadui_displaymode", "0", FCVAR_NONE, "",
                              OnResolutionsNeedUpdate);
...
public:
  ...
  void SetOptionDescription(GamepadUIString *pStr) {
    m_strOptionDescription = pStr;
  }
  ...
  MESSAGE_FUNC_HANDLE(OnGamepadUIButtonNavigatedTo,
                      "OnGamepadUIButtonNavigatedTo", button);

  static GamepadUIOptionsPanel *GetInstance() { return s_pOptionsPanel; }

  GamepadUIWheelyWheel *GetResolutionButton() { return m_pResolutionButton; }

private:
  GAMEPADUI_PANEL_PROPERTY(float, m_flTabsOffsetX, "Tabs.OffsetX", "0",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flTabsOffsetY, "Tabs.OffsetY", "0",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flOptionsFade, "Options.Fade", "80",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flScrollBarOffsetX,
                           "Options.Scrollbar.OffsetX", "716",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flScrollBarOffsetY,
                           "Options.Scrollbar.OffsetY", "128",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flScrollBarHeight,
                           "Options.Scrollbar.Height", "256",
                           SchemeValueTypes::ProportionalFloat);
...
ConVar gamepadui_last_options_tab("gamepadui_last_options_tab", "0",
                                  FCVAR_ARCHIVE);
...
public:
  GamepadUIOptionButton(vgui::Panel *pParent, vgui::Panel *pActionSignalTarget,
  ...
  GamepadUIOptionButton(vgui::Panel *pParent, vgui::Panel *pActionSignalTarget,
                        const char *pSchemeFile, const char *pCommand,
                        const wchar_t *pText, const wchar_t *pDescription)
      : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription) {}
  ...
  void SetArmed(bool state) OVERRIDE {
    ...
    if (state && ShowDescriptionAtFooter()) {
      ...
      GamepadUIOptionsPanel::GetInstance()->SetOptionDescription(
          &m_strButtonDescription);
      m_bDescriptionHide = true;
    }
  }

  void SetHorizontal(bool bHorz) { m_bHorizontal = bHorz; }
...
public:
  GamepadUIHeaderButton(vgui::Panel *pParent, vgui::Panel *pActionSignalTarget,
                        const char *pSchemeFile, const char *pCommand,
                        const char *pText, const char *pDescription)
      : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription) {}
  ...
  void SetCentered(bool bHorz) { m_bCenter = bHorz; }

  bool IsCentered() { return m_bCenter; }
...
public:
  ...
  GamepadUICheckButton(vgui::Panel *pParent, vgui::Panel *pActionSignalTarget,
                       const char *pSchemeFile, const char *pCommand,
                       const char *pText, const char *pDescription)
      : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription) {}
  ...
  virtual void Paint() {
    ...
    vgui::surface()->DrawSetTextPos(m_flCheckOffsetX,
                                    m_flHeight / 2 - m_flCheckHeight / 2);
...
private:
  GAMEPADUI_PANEL_PROPERTY(float, m_flCheckOffsetX, "Button.Check.OffsetX",
                           "10", SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flCheckHeight, "Button.Check.Height", "18",
                           SchemeValueTypes::ProportionalFloat);
...
public:
  ...
  GamepadUIKeyButton(const char *pszBinding, vgui::Panel *pParent,
                     vgui::Panel *pActionSignalTarget, const char *pSchemeFile,
                     const char *pCommand, const char *pText,
  ...
  void Paint() OVERRIDE {
    ...
    if (m_bBeingBound) {
      ...
      vgui::surface()->DrawFilledRect(
          m_flWidth - m_flTextOffsetX - m_flBindingWidth,
          m_flHeight / 2 - m_flBindingHeight / 2, m_flWidth - m_flTextOffsetX,
          m_flHeight / 2 + m_flBindingHeight / 2);
    } else {
      ...
      vgui::surface()->DrawSetTextFont(
          state == ButtonStates::Out ? m_hTextFont : m_hTextFontOver);
      vgui::surface()->GetTextSize(state == ButtonStates::Out ? m_hTextFont
                                                              : m_hTextFontOver,
                                   wszBuffer, nTextSizeX, nTextSizeY);

      vgui::surface()->DrawSetTextPos(m_flWidth - m_flTextOffsetX - nTextSizeX,
                                      m_flHeight / 2 - nTextSizeY / 2);
  ...
  void ApplySchemeSettings(vgui::IScheme *pScheme) {
    ...
    // (This would be more fitting for the release sound, but this class reuses
    // the slider res file, which doesn't normally use a release sound)
  ...
  void OnThink() OVERRIDE {
    ...
    if (m_bBeingBound) {
      ...
      if (GamepadUI::GetInstance().GetEngineClient()->CheckDoneKeyTrapping(
              code)) {
        OnKeyBound(code);
      }
    }
...
protected:
  ...
  GAMEPADUI_PANEL_PROPERTY(float, m_flBindingWidth, "Button.Binding.Width",
                           "160", SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flBindingHeight, "Button.Binding.Height",
                           "11", SchemeValueTypes::ProportionalFloat);

  GAMEPADUI_PANEL_PROPERTY(Color, m_colBinding, "Button.Binding", "0 0 0 255",
                           SchemeValueTypes::Color);
};
...
public:
  ...
  GamepadUIConvarButton(const char *pszCvar, const char *pszCvarDepends,
                        bool bInstantApply, vgui::Panel *pParent,
                        vgui::Panel *pActionSignalTarget,
                        const char *pSchemeFile, const char *pCommand,
                        const char *pText, const char *pDescription)
      : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription),
        m_cvar(pszCvar), m_szDependentCVar(pszCvarDepends),
  ...
  const char *GetDependentCVar() const { return m_szDependentCVar.String(); }

  const char *GetConVarName() const { return m_cvar.GetName(); }
...
public:
  ...
  GamepadUIWheelyWheel(const char *pszCvar, const char *pszCvarDepends,
                       bool bInstantApply, bool bSignOnly, vgui::Panel *pParent,
                       vgui::Panel *pActionSignalTarget,
                       const char *pSchemeFile, const char *pCommand,
                       const char *pText, const char *pDescription)
      : BaseClass(pszCvar, pszCvarDepends, bInstantApply, pParent,
                  pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription),
  ...
  void OnKeyCodePressed(vgui::KeyCode code) {
    ...
    case STEAMCONTROLLER_DPAD_LEFT:
      ...
      vgui::surface()->PlaySound(
          g_ButtonSoundNames.String(m_sDepressedSoundName));
      DecrementValue();
      break;
  ...
  void OnMousePressed(vgui::MouseCode code) {
    ...
    int nTextW, nTextH;
    vgui::surface()->GetTextSize(m_hTextFont, strOption.String(), nTextW,
                                 nTextH);

    int nScrollerSize = vgui::surface()->GetCharacterWidth(m_hTextFont, L'<') +
                        vgui::surface()->GetCharacterWidth(m_hTextFont, L' ');
  ...
  void ShowWarning(int nIndex, bool bIncrement) {
    new GamepadUIGenericConfirmationPanel(
        GamepadUIOptionsPanel::GetInstance(), "OptionWarning",
        GamepadUIString("#GameUI_Confirm").String(),
        m_DangerousOptionsText[nIndex].String(),
        [this, bIncrement]() {
          bIncrement ? this->IncrementValueInner()
                     : this->DecrementValueInner();
        },
        true, true);
  }
  ...
  void UpdateConVar() OVERRIDE {
    if (IsDirty()) {
      if (m_bSignOnly)
        m_cvar.SetValue(abs(m_cvar.GetFloat()) *
                        m_Options[m_nSelectedItem].nValue);
      else
  ...
  bool IsDirty() OVERRIDE {
    return m_cvar.IsValid() &&
           GetCvarValue() != m_Options[m_nSelectedItem].nValue;
  }
  ...
  virtual void Paint() {
    ...
    if (state != ButtonStates::Out) {
      vgui::surface()->DrawSetTextPos(
          m_flWidth - m_flTextOffsetX - nTextW - 2 * nScrollerSize, nTextY);
      vgui::surface()->DrawPrintText(L"< ", 2);
    }
    vgui::surface()->DrawSetTextPos(
        m_flWidth - m_flTextOffsetX - nTextW - nScrollerSize, nTextY);
    vgui::surface()->DrawPrintText(strOption.String(), strOption.Length());
    if (state != ButtonStates::Out) {
      vgui::surface()->DrawSetTextPos(
          m_flWidth - m_flTextOffsetX - nScrollerSize, nTextY);
      vgui::surface()->DrawPrintText(L" >", 2);
    }
  }

  void ClearOptions() { m_Options.RemoveAll(); }

  void AddOptionItem(GamepadUIOption option) { m_Options.AddToTail(option); }

  int GetOptionCount() { return m_Options.Count(); }
...
private:
  ...
  GAMEPADUI_PANEL_PROPERTY(float, m_flClickPadding, "Button.Wheel.ClickPadding",
                           "24", SchemeValueTypes::ProportionalFloat);
...
public:
  ...
  GamepadUISlideySlide(const char *pszCvar, const char *pszCvarDepends,
                       bool bInstantApply, float flMin, float flMax,
                       float flStep, int nTextPrecision, vgui::Panel *pParent,
                       vgui::Panel *pActionSignalTarget,
                       const char *pSchemeFile, const char *pCommand,
                       const char *pText, const char *pDescription)
      : BaseClass(pszCvar, pszCvarDepends, bInstantApply, pParent,
                  pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription),
        m_flMin(flMin), m_flMax(flMax), m_flStep(flStep),
  ...
  void OnKeyCodePressed(vgui::KeyCode code) {
    ...
    {
      float flValue =
          Clamp(m_flValue - (m_bFineAdjust ? m_flMouseStep : m_flStep), m_flMin,
                m_flMax);
      if (flValue != m_flValue) {
        if (m_sSliderSoundName != UTL_INVAL_SYMBOL)
          vgui::surface()->PlaySound(
              g_ButtonSoundNames.String(m_sSliderSoundName));
        m_flValue = flValue;
        if (m_bInstantApply)
          UpdateConVar();
      }
  ...
  void SetMouseStep(float flStep) { m_flMouseStep = flStep; }
  ...
  void OnThink() {
    if (IsSelected()) {
      ...
      float flProgress =
          RemapValClamped(mx, iSliderStart, iSliderEnd, m_flMin, m_flMax);
      ...
      if (flProgress != m_flValue) {
        if (m_sSliderSoundName != UTL_INVAL_SYMBOL &&
            m_flLastSliderSoundTime < GamepadUI::GetInstance().GetTime()) {
          vgui::surface()->PlaySound(
              g_ButtonSoundNames.String(m_sSliderSoundName));
          m_flLastSliderSoundTime = GamepadUI::GetInstance().GetTime() +
                                    0.04f; // Arbitrary sound cooldown
        }
  ...
  void ApplySchemeSettings(vgui::IScheme *pScheme) {
    ...
    const char *pSliderSound =
        pScheme->GetResourceString("Slider.Sound.Adjust");
  ...
  virtual void Paint() {
    ...
    if (nTextPrecision >= 0) {
      ...
      vgui::surface()->DrawSetTextPos(m_flWidth - 2 * m_flTextOffsetX -
                                          m_flSliderWidth - w,
                                      m_flHeight / 2 - h / 2);
    ...
    vgui::surface()->DrawFilledRect(
        m_flWidth - m_flTextOffsetX - m_flSliderWidth,
        m_flHeight / 2 - m_flSliderHeight / 2, m_flWidth - m_flTextOffsetX,
        m_flHeight / 2 + m_flSliderHeight / 2);

    float flFill = m_flSliderWidth * (1.0f - GetMultiplier());

    vgui::surface()->DrawSetColor(m_colSliderFill);
    vgui::surface()->DrawFilledRect(m_flWidth - m_flTextOffsetX -
                                        m_flSliderWidth,
                                    m_flHeight / 2 - m_flSliderHeight / 2,
  ...
  void RunAnimations(ButtonState state) OVERRIDE {
    ...
    GAMEPADUI_RUN_ANIMATION_COMMAND(
        m_colSliderBacking, vgui::AnimationController::INTERPOLATOR_LINEAR);
    GAMEPADUI_RUN_ANIMATION_COMMAND(
        m_colSliderFill, vgui::AnimationController::INTERPOLATOR_LINEAR);
  }
...
private:
  ...
  GAMEPADUI_BUTTON_ANIMATED_PROPERTY(Color, m_colSliderBacking,
                                     "Slider.Backing", "255 255 255 22",
                                     SchemeValueTypes::Color);
  GAMEPADUI_BUTTON_ANIMATED_PROPERTY(Color, m_colSliderFill, "Slider.Fill",
                                     "255 255 255 255",
                                     SchemeValueTypes::Color);

  GAMEPADUI_PANEL_PROPERTY(float, m_flSliderWidth, "Slider.Width", "160",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flSliderHeight, "Slider.Height", "11",
                           SchemeValueTypes::ProportionalFloat);
};
...
public:
  ...
  GamepadUISkillySkill(vgui::Panel *pParent, vgui::Panel *pActionSignalTarget,
                       const char *pSchemeFile, const char *pCommand,
                       const char *pText, const char *pDescription,
                       const char *pImage, int nSkill)
      : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription),
        m_Image(pImage), m_nSkill(nSkill) {}

  GamepadUISkillySkill(vgui::Panel *pParent, vgui::Panel *pActionSignalTarget,
                       const char *pSchemeFile, const char *pCommand,
                       const wchar *pText, const wchar *pDescription,
                       const char *pImage, int nSkill)
      : BaseClass(pParent, pActionSignalTarget, pSchemeFile, pCommand, pText,
                  pDescription),
        m_Image(pImage), m_nSkill(nSkill) {}
  ...
  void RunAnimations(ButtonState state) OVERRIDE {
    ...
    GAMEPADUI_RUN_ANIMATION_COMMAND(
        m_colTitleBackground, vgui::AnimationController::INTERPOLATOR_LINEAR);
    GAMEPADUI_RUN_ANIMATION_COMMAND(
        m_colDescriptionBackground,
        vgui::AnimationController::INTERPOLATOR_LINEAR);
    GAMEPADUI_RUN_ANIMATION_COMMAND(
        m_colImage, vgui::AnimationController::INTERPOLATOR_LINEAR);
  }
  ...
  void Paint() OVERRIDE {
    ...
    vgui::surface()->GetTextSize(
        GetCurrentButtonState() == ButtonStates::Out ? m_hTextFont
                                                     : m_hTextFontOver,
        m_strButtonText.String(), nTextSizeX, nTextSizeY);
    int y2 = m_flTextOffsetY + m_flHeight / 2 + nTextSizeY / 2;

    vgui::surface()->DrawSetColor(m_colTitleBackground);
    vgui::surface()->DrawFilledRect(
        0, m_flTextOffsetY + m_flHeight / 2 - nTextSizeY / 2,
        nTextSizeX + m_flTextOffsetX * 2, y2);

    vgui::surface()->DrawSetColor(m_colDescriptionBackground);
    vgui::surface()->DrawFilledRect(0, y2, m_flWidth,
                                    m_flHeight + m_flExtraHeight);

    vgui::surface()->DrawSetColor(m_colImage);
    vgui::surface()->DrawSetTexture(m_Image);
    vgui::surface()->DrawTexturedRect(m_flWidth / 2 - m_flImageWidth / 2, 0,
                                      m_flWidth / 2 + m_flImageWidth / 2,
                                      m_flImageHeight);
...
private:
  ...
  GAMEPADUI_PANEL_PROPERTY(float, m_flImageWidth, "Button.Image.Width", "40",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_PANEL_PROPERTY(float, m_flImageHeight, "Button.Image.Height", "40",
                           SchemeValueTypes::ProportionalFloat);
  GAMEPADUI_BUTTON_ANIMATED_PROPERTY(Color, m_colImage, "Button.Image",
                                     "255 255 255 255",
                                     SchemeValueTypes::Color);
  GAMEPADUI_BUTTON_ANIMATED_PROPERTY(Color, m_colTitleBackground,
                                     "Button.Title.Background", "0 0 0 0",
                                     SchemeValueTypes::Color);
  GAMEPADUI_BUTTON_ANIMATED_PROPERTY(Color, m_colDescriptionBackground,
                                     "Button.Description.Background", "0 0 0 0",
                                     SchemeValueTypes::Color);
};
...
int GetCurrentAntialiasing() {
  ...
  for (int nAAMode = 0; nAAMode < g_nNumAAModes; nAAMode++) {
    ...
    // index
    if ((g_AAModes[nAAMode].m_nNumSamples == nAASamples) &&
        (g_AAModes[nAAMode].m_nQualityLevel == nAAQuality)) {
      return nAAMode;
    }
  }
...
int GetCurrentAspectRatio() {
  ...
  return GetScreenAspectMode(config.m_VideoMode.m_Width,
...
int GetCurrentDisplayMode() {
  const MaterialSystem_Config_t &config =
      materials->GetCurrentConfigForVideoCard();
  if (config.Windowed()) {
#ifdef HL2_RETAIL // SDK2013 does not natively support borderless windowed
                  // (Madi)
...
#else
  // TODO FIXME: SDK2013 uses the inverse here...1 is "windowed" and 0 is
  // "fullscreen" in materialsystem. but our values mean the opposite in
  // gamepadUI code. (Madi)
...
void FlushPendingAntialiasing() {
  ...
  int nAAMode =
      Clamp(_gamepadui_antialiasing.GetInt(), 0, Max(0, g_nNumAAModes - 1));
...
void FlushPendingResolution() {
  ...
  GamepadUIOption *pOption =
      pResButton->GetOption(_gamepadui_resolution.GetInt());
  if (!pOption)
    return;
  ...
  if (pOption->userdata.nWidth != config.m_VideoMode.m_Width ||
      pOption->userdata.nHeight != config.m_VideoMode.m_Height)
    bDirty = true;
  ...
  // "fullscreen" in materialsystem. but our values mean the opposite in
  // gamepadUI code. (Madi)
  const int nWindowed = _gamepadui_displaymode.GetInt() == 1 ? 0 /* fullscreen*/
                                                             : 1 /* windowed */;
#endif
...
GamepadUIOptionsPanel::GamepadUIOptionsPanel(vgui::Panel *pParent,
                                             const char *pPanelName)
    : BaseClass(pParent, pPanelName) {
  s_pOptionsPanel = this;

  vgui::HScheme Scheme = vgui::scheme()->LoadSchemeFromFileEx(
      GamepadUI::GetInstance().GetSizingVPanel(),
      GAMEPADUI_DEFAULT_PANEL_SCHEME, "SchemePanel");
  SetScheme(Scheme);
  ...
  m_pScrollBar = new GamepadUIScrollBar(
      this, this, GAMEPADUI_RESOURCE_FOLDER "schemescrollbar.res", NULL, false);
}
...
void GamepadUIOptionsPanel::Paint() {
  ...
  const int nLastTabX =
      m_flTabsOffsetX + m_nTabCount * m_Tabs[0].pTabButton->GetWide();
  ...
  if (m_leftGlyph.SetupGlyph(nGlyphSize, "menu_lb", true))
    m_leftGlyph.PaintGlyph(m_flTabsOffsetX - nGlyphSize - nGlyphOffsetX,
                           m_flTabsOffsetY + nGlyphOffsetY / 2, nGlyphSize,
                           255);

  if (m_rightGlyph.SetupGlyph(nGlyphSize, "menu_rb", true))
    m_rightGlyph.PaintGlyph(nLastTabX + nGlyphOffsetX,
                            m_flTabsOffsetY + nGlyphOffsetY / 2, nGlyphSize,
                            255);
  ...
  if (m_strOptionDescription != NULL) {
    ...
    float flX = m_flFooterButtonsOffsetX + m_nFooterButtonWidth +
                m_flFooterButtonsSpacing;
    ...
    int nMaxWidth = nParentW - flX -
                    (m_flFooterButtonsOffsetX + m_nFooterButtonWidth +
                     m_flFooterButtonsSpacing);

    DrawPrintWrappedText(m_hDescFont, flX, flY,
                         m_strOptionDescription->String(),
                         m_strOptionDescription->Length(), nMaxWidth, true);
  }
}
...
void GamepadUIOptionsPanel::UpdateGradients() {
  ...
  GamepadUI::GetInstance().GetGradientHelper()->SetTargetGradient(
      GradientSide::Up, {1.0f, 1.0f}, flTime);
  GamepadUI::GetInstance().GetGradientHelper()->SetTargetGradient(
      GradientSide::Down, {1.0f, 0.5f}, flTime);
}
...
void GamepadUIOptionsPanel::LayoutCurrentTab() {
  ...
  for (GamepadUIOptionButton *pButton : m_Tabs[nActiveTab].pButtons) {
    ...
    {
      buttonY = y + previousySizes -
                m_Tabs[nActiveTab].ScrollState.GetScrollProgress();
      if (buttonY < y)
        fade = RemapValClamped(
            y - buttonY, abs(m_flOptionsFade - pButton->GetTall()), 0, 0, 255);
      else if (buttonY > (nParentH - m_flFooterButtonsOffsetY -
                          m_nFooterButtonHeight - m_flOptionsFade))
        fade = RemapValClamped(
            (nParentH - m_flFooterButtonsOffsetY - m_nFooterButtonHeight) -
                (buttonY + pButton->GetTall()),
            0, m_flOptionsFade, 0, 255);
      if ((pButton->HasFocus() && pButton->IsEnabled()) && fade != 0)
        fade = 255;
    }

    GamepadUIConvarButton *pCvarButton =
        dynamic_cast<GamepadUIConvarButton *>(pButton);
    ...
    if (pCvarButton) {
      ...
      if (pszDependentCVar && *pszDependentCVar) {
        ...
        for (GamepadUIButton *pOtherButton : m_Tabs[nActiveTab].pButtons) {
          GamepadUIConvarButton *pOtherCvarButton =
              dynamic_cast<GamepadUIConvarButton *>(pOtherButton);
          if (pOtherCvarButton) {
            const char *pszConVarName = pOtherCvarButton->GetConVarName();
            if (pszConVarName && *pszConVarName &&
                !V_strcmp(pszDependentCVar, pszConVarName)) {
    ...
    if (pButton->IsEnabled() && pButton->IsVisible() && fade &&
        m_Tabs[nActiveTab].bAlternating) {
      buttonWide = pButton->GetWide();
      if (i % 2)
        vgui::surface()->DrawSetColor(
            Color(0, 0, 0, (20 * Min(255, fade + 127)) / 255));
      else
        vgui::surface()->DrawSetColor(
            Color(fade, fade, fade, fade > 64 ? 1 : 0));

      vgui::surface()->DrawFilledRect(buttonX, buttonY, buttonX + buttonWide,
                                      buttonY + pButton->GetTall());
    }
  ...
  int yMax = 0;
  {
    ...
    m_pScrollBar->UpdateScrollBounds(0.0f, yMax, m_flScrollBarHeight,
                                     m_flScrollBarHeight);

    m_Tabs[nActiveTab].ScrollState.UpdateScrollBounds(0.0f, yMax);
  }

  m_Tabs[nActiveTab].ScrollState.UpdateScrolling(
      2.0f, GamepadUI::GetInstance().GetTime());
}

void GamepadUIOptionsPanel::OnMouseWheeled(int delta) {
  m_Tabs[GetActiveTab()].ScrollState.OnMouseWheeled(
      delta * 100.0f, GamepadUI::GetInstance().GetTime());
}
...
void GamepadUIOptionsPanel::OnCommand(char const *pCommand) {
  if (!V_strcmp(pCommand, "action_back")) {
    ...
    for (int i = 0; i < m_Tabs[GetActiveTab()].pButtons.Count(); i++) {
      GamepadUIConvarButton *pConVarButton =
          dynamic_cast<GamepadUIConvarButton *>(
              m_Tabs[GetActiveTab()].pButtons[i]);
      if (pConVarButton)
    ...
    new GamepadUIGenericConfirmationPanel(
        GamepadUIOptionsPanel::GetInstance(), "UseDefaultsConfirm",
        GamepadUIString("#GameUI_KeyboardSettings").String(),
        GamepadUIString("#GameUI_KeyboardSettingsText").String(),
        []() {
          GamepadUI::GetInstance().GetEngineClient()->ClientCmd_Unrestricted(
    ...
    if (GamepadUI::GetInstance().GetSteamInput()->IsEnabled()) {
      GamepadUI::GetInstance().GetSteamInput()->ShowBindingPanel(
          GamepadUI::GetInstance().GetSteamInput()->GetActiveController());
    }
#elif defined(HL2_RETAIL) // Steam input and Steam Controller are not supported
                          // in SDK2013 (Madi)
    ...
    if (!nController) {
      ...
      GamepadUI::GetInstance()
          .GetSteamAPIContext()
          ->SteamInput()
          ->GetConnectedControllers(nControllers);
...
#endif
    new GamepadUIGenericConfirmationPanel(
        GamepadUIOptionsPanel::GetInstance(), "TechCredits", title.String(),
        wszBuf, []() {}, true, false);
...
const char *UTIL_Parse(const char *data, char *token, int sizeofToken) {
  data = GamepadUI::GetInstance().GetEngineClient()->ParseFile(data, token,
                                                               sizeofToken);
  return data;
}
...
static void GetCurrentAndDesktopSize(int &currentWidth, int &currentHeight,
                                     int &desktopWidth, int &desktopHeight) {
  const MaterialSystem_Config_t &config =
      materials->GetCurrentConfigForVideoCard();
  ...
  GamepadUI::GetInstance().GetGameUIFuncs()->GetDesktopResolution(
  ...
  bool bNewFullscreenDisplay =
      (!bWindowed && (GetSDLDisplayIndexFullscreen() != nNewDisplayIndex));
...
void GamepadUIOptionsPanel::UpdateResolutions() {
  ...
  GetCurrentAndDesktopSize(currentWidth, currentHeight, desktopWidth,
                           desktopHeight);
...
void GamepadUIOptionsPanel::ClearBindings() {
  for (int i = 0; i < m_nTabCount; i++) {
    for (GamepadUIButton *pButton : m_Tabs[i].pButtons) {
      GamepadUIKeyButton *pKeyButton =
          dynamic_cast<GamepadUIKeyButton *>(pButton);
      if (pKeyButton)
        pKeyButton->ClearKey();
    }
  }
...
void GamepadUIOptionsPanel::FillInBindings() {
  ...
  for (int i = 0; i < BUTTON_CODE_LAST; i++) {
    // Look up binding
    const char *binding =
        GamepadUI::GetInstance().GetGameUIFuncs()->GetBindingForButtonCode(
            (ButtonCode_t)i);
    if (!binding)
      continue;
    ...
    for (int j = 0; j < m_nTabCount; j++) {
      for (GamepadUIButton *pFindButton : m_Tabs[j].pButtons) {
        GamepadUIKeyButton *pFindKeyButton =
            dynamic_cast<GamepadUIKeyButton *>(pFindButton);
        if (pFindKeyButton &&
            !V_strcmp(pFindKeyButton->GetKeyBinding(), binding)) {
    ...
    if (pButton) {
      ...
      if (*pCurrentKey) {
        ButtonCode_t currentBC = (ButtonCode_t)GamepadUI::GetInstance()
                                     .GetGameUIFuncs()
                                     ->GetButtonCodeForBind(pCurrentKey);
...
void GamepadUIOptionsPanel::OnGamepadUIButtonNavigatedTo(vgui::VPANEL button) {
  GamepadUIButton *pButton = dynamic_cast<GamepadUIButton *>(
      vgui::ipanel()->GetPanel(button, GetModuleName()));
  if (pButton) {
    int nParentW, nParentH;
    GetParent()->GetSize(nParentW, nParentH);

    const float flTabButtonHeight =
        m_Tabs[GetActiveTab()].pTabButton->m_flHeight;

    float flScrollRegion =
        nParentH - (m_flTabsOffsetY + m_flFooterButtonsOffsetY +
                    m_nFooterButtonHeight + flTabButtonHeight);
    int nX, nY;
    pButton->GetPos(nX, nY);
    if (nY + m_flFooterButtonsOffsetY + m_nFooterButtonHeight +
                pButton->GetTall() >
            nParentH ||
        nY < m_flTabsOffsetY + flTabButtonHeight) {
      int nTargetY = 0;
      int nThisButton = -1;
      int nHeader = -1;
      int nHorzHeight = 0;
      ...
      // If this button has a section header above it and we're going up, scroll
      // to it
      ...
      m_Tabs[GetActiveTab()].ScrollState.SetScrollTarget(
          nTargetY, GamepadUI::GetInstance().GetTime());
    }
  }
}
...
void GamepadUIOptionsPanel::LoadOptionTabs(const char *pszOptionsFile) {
  ...
  if (pDataFile->LoadFromFile(g_pFullFileSystem, pszOptionsFile)) {
    for (KeyValues *pTabData = pDataFile->GetFirstSubKey(); pTabData != NULL;
         pTabData = pTabData->GetNextKey()) {
      {
        char buttonCmd[64];
        V_snprintf(buttonCmd, sizeof(buttonCmd), "tab %d", m_nTabCount);

        auto button = new GamepadUIButton(
            this, this, GAMEPADUI_RESOURCE_FOLDER "schemetab.res", buttonCmd,
      ...
      if (pTabItems) {
        for (KeyValues *pItemData = pTabItems->GetFirstSubKey();
             pItemData != NULL; pItemData = pItemData->GetNextKey()) {
          ...
          if (!V_strcmp(pItemType, "checkybox")) {
            auto button = new GamepadUICheckButton(
                ...
                "button_pressed", pItemData->GetString("text", ""),
                pItemData->GetString("description", ""));
            m_Tabs[m_nTabCount].pButtons.AddToTail(button);
            ...
            auto button = new GamepadUISkillySkill(
                ...
                V_atoi(pItemData->GetString("skill", "")));
            button->SetMouseNavigate(false);
            button->SetHorizontal(pItemData->GetBool(
                "horizontal", m_Tabs[m_nTabCount].bHorizontal));
            m_Tabs[m_nTabCount].pButtons.AddToTail(button);
            ...
            auto button = new GamepadUISlideySlide(
                pszCvar, pszCvarDepends, bInstantApply, flMin, flMax, flStep,
                nTextPrecision, this, this,
            ...
            auto button = new GamepadUIWheelyWheel(
                pszCvar, pszCvarDepends, bInstantApply, bSignOnly, this, this,
                GAMEPADUI_RESOURCE_FOLDER "schemeoptions_wheelywheel.res",
                "button_pressed", pItemData->GetString("text", ""),
                pItemData->GetString("description", ""));
            ...
            if (pOptions) {
              for (KeyValues *pOptionData = pOptions->GetFirstSubKey();
                   pOptionData != NULL;
                   pOptionData = pOptionData->GetNextKey()) {
                GamepadUIOption option;
                option.nValue = V_atoi(pOptionData->GetName());
                option.strOptionText =
                    GamepadUIString(pOptionData->GetString());
              ...
              if (!V_strcmp(pszOptionsFrom, "antialiasing")) {
                ...
                V_swprintf_safe(wszNoBorderText, L"%ls (No Border)",
                                option.strOptionText.String());
                option.nValue = i++;
                ...
                if (numVideoDisplays <= 1) {
                  ...
                  for (int display = 0; display < numVideoDisplays; display++) {
                    ...
                    V_swprintf_safe(wszItemText, L"%ls (%d)",
                                    strFullscreen.String(), display);
            ...
            if (pConfirm) {
              for (KeyValues *pConfirmData = pConfirm->GetFirstSubKey();
                   pConfirmData != NULL;
                   pConfirmData = pConfirmData->GetNextKey()) {
                button->AddDangerousOption(V_atoi(pConfirmData->GetName()),
                                           pConfirmData->GetString());
              }
            }
