...
void CC_ResetFade() { ResetFade = true; }
...
GamepadUIMainMenu::GamepadUIMainMenu(vgui::Panel *pParent)
  ...
  if (pModData) {
    if (pModData->LoadFromFile(g_pFullFileSystem, "gameinfo.txt")) {
      m_LogoText[0].SetText(
          pModData->GetString("gamepadui_title", pModData->GetString("title")));
      m_LogoText[1].SetText(pModData->GetString("gamepadui_title2",
...
void GamepadUIMainMenu::UpdateGradients() {
  ...
  GamepadUI::GetInstance().GetGradientHelper()->SetTargetGradient(
      GradientSide::Left,
      {1.0f, GamepadUI::GetInstance().IsInBackgroundLevel() ? 0 : 0.666f},
      flTime);
  // #else
  //     GamepadUI::GetInstance().GetGradientHelper()->SetTargetGradient(
...
void GamepadUIMainMenu::LoadMenuButtons() {
  ...
  if (pDataFile) {
    if (pDataFile->LoadFromFile(g_pFullFileSystem, GAMEPADUI_MAINMENU_FILE)) {
      for (KeyValues *pData = pDataFile->GetFirstSubKey(); pData != NULL;
           pData = pData->GetNextKey()) {
        GamepadUIButton *pButton = new GamepadUIButton(
            this, this, GAMEPADUI_MAINMENU_SCHEME, pData->GetString("command"),
        ...
        // pButton->SetSize(pButton->GetWide() / BUTTONS_DEVIDE_SIZE ,
        // pButton->GetTall() / BUTTONS_DEVIDE_SIZE);
...
void GamepadUIMainMenu::LayoutMainMenu() {
  ...
  // HACK if we have more than 0.7 sec of delay between frames, we possybly
  // lagging
  ...
  for (GamepadUIButton *pButton : currentButtons) {
    ...
    // FIXME ive tried to make this anim seperatly for each button so they
    // whould move next to each other, but the life planned other plans, so when
    // im subtracting 0.5*i it stopping them at some distance and they dont go
    // all the way.
    float TdC;
    ...
    if (GetCurrentMenuState() == GamepadUIMenuStates::InGame &&
        !GamepadUI::GetInstance().IsInBackgroundLevel()) {
      TdC = clamp(curtime, 0, m_flButtonsAnimTimeInGame);
      func = clamp(
          pow((TdC / m_flButtonsAnimTimeInGame), m_flButtonsAnimPowerInGame) /
              (pow((TdC / m_flButtonsAnimTimeInGame),
                   m_flButtonsAnimPowerInGame) +
               pow(1 - (TdC / m_flButtonsAnimTimeInGame),
                   m_flButtonsAnimPowerInGame)),
          0, 1);
    } else {
      TdC = clamp(curtime, 0, m_flButtonsAnimTime);
      func = clamp(
          pow((TdC / m_flButtonsAnimTime), m_flButtonsAnimPower) /
              (pow((TdC / m_flButtonsAnimTime), m_flButtonsAnimPower) +
               pow(1 - (TdC / m_flButtonsAnimTime), m_flButtonsAnimPower)),
          0, 1);
    }

    m_flButtonsRealOffsetX =
        RemapVal(func, 0, 1, m_flButtonsStartOffsetX, m_flButtonsOffsetX);
...
void GamepadUIMainMenu::PaintLogo() {
#ifdef GAMEPADUI_GAME_HL2 // a little hack to make default hl2 logo be yellow
                          // and still be able to change it later in res file
  ...
  if (m_LogoImage.IsValid()) {
    ...
    // m_flLogoRealOffsetX = Lerp<float>(m_flLogoLerp * TimeDeltaLogo,
    // m_flLogoRealOffsetX, m_flLogoOffsetX);
    float func;
    ...
    if (GetCurrentMenuState() == GamepadUIMenuStates::InGame) {
      ...
      func = clamp(
          pow((TdC / m_flLogoAnimTimeInGame), m_flLogoAnimPowerInGame) /
              (pow((TdC / m_flLogoAnimTimeInGame), m_flLogoAnimPowerInGame) +
               pow(1 - (TdC / m_flLogoAnimTimeInGame),
                   m_flLogoAnimPowerInGame)),
          0, 1);
    } else {
      TdC = clamp(curtime, 0, m_flLogoAnimTime);
      func = clamp(pow((TdC / m_flLogoAnimTime), m_flLogoAnimPower) /
                       (pow((TdC / m_flLogoAnimTime), m_flLogoAnimPower) +
                        pow(1 - (TdC / m_flLogoAnimTime), m_flLogoAnimPower)),
                   0, 1);
    }
