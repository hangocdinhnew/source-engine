...
class CWeaponGravityGun
    : public CBaseHLCombatWeapon // Formerly CBaseCombatWeapon.
{
  DECLARE_DATADESC();

public:
  DECLARE_CLASS(CWeaponGravityGun,
                CBaseHLCombatWeapon); // Formerly CBaseCombatWeapon.
...
"info_player_start"); Vector start = pSpot->GetAbsOrigin(); static Vector
*targets = NULL; static bool first = true; static float test[2] = {1,1}; if (
first )
        {
...
g_PhysWorldObject->GetCollide(); trace_t tr;

                        for ( i = 0; i < NUM_COLLISION_TESTS; i++ )
                        {
                                physcollision->TraceBox( start, targets[i],
-size[j], size[j], pCollide, vec3_origin, vec3_angle, &tr ); dots +=
physcollision->ReadStat(0);
                                //results[i] = tr.endpos;
                        }
                }
                else
                {
                        ...
                        for ( i = 0; i < NUM_COLLISION_TESTS; i++ )
                        {
                                UTIL_TraceModel( start, targets[i], -size[j],
size[j], pWorld, COLLISION_GROUP_NONE, &tr );
                                //results[i] = tr.endpos;
                        }
                }
...
duration*1000, dots ); Msg("Current speed ratio: %.2fX BSP:JGJK\n", test[1] /
test[0] ); #if 0 int red = 255, green = 0, blue = 0; for ( i = 0; i <
NUM_COLLISION_TESTS; i++ )
        {
                NDebugOverlay::Line( start, results[i], red, green, blue, false,
2 );
        }
#endif
}
static ConCommand collision_test("collision_test", CC_CollisionTest, "Tests
collision system", FCVAR_CHEAT );
*/
