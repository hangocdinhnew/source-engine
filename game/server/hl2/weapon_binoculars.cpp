...
enum ZoomMode_t { Zoom_Exit = 0, Zoom_In, Zoom_Out };

//-----------------------------------------------------------------------------
// Discrete zoom levels for the scope.
//-----------------------------------------------------------------------------
static int g_nZoomFOV[] = {0, 40, 30, 20, 10, 5};
...
DEFINE_FIELD(m_NextCacheTime, FIELD_TIME), DEFINE_SOUNDPATCH(m_pSoundSignal),
    DEFINE_SOUNDPATCH(m_pSoundStatic), DEFINE_FIELD(m_fNextZoom, FIELD_FLOAT),
...
bool CWeaponBinoculars::Deploy() {
  ...
  if (!m_pSoundStatic || !m_pSoundSignal) {
    ...
    m_pSoundStatic = controller.SoundCreate(
        filter, entindex(), WB_STATIC_CHANNEL, pStaticSounds[0], ATTN_NONE);
    m_pSoundSignal = controller.SoundCreate(
        filter, entindex(), WB_SIGNAL_CHANNEL, pSignalSounds[0], ATTN_NONE);
  }
...
// Output : int
//-----------------------------------------------------------------------------
int CWeaponBinoculars::CapabilitiesGet(void) { return 0; }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponBinoculars::GetSoundDist(CBaseEntity *pSound,
                                      const Vector &vecLOS) {
  CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
  if (!pPlayer)
    return 0.0f;
...
CBaseEntity *CWeaponBinoculars::LocateBestSound(void) {
  // go through the sound list and find the sound the player is probably trying
  // to listen to.
  EHANDLE *pIterate;
...
void CWeaponBinoculars::ItemPostFrame(void) {
  ...
  /*
        ...
        // No buttons down.
        //
        if (!(( pPlayer->m_nButtons & IN_ATTACK ) || ( pPlayer->m_nButtons &
     IN_ATTACK2 )))
        {
                WeaponIdle();
                return;
...
void CWeaponBinoculars::Precache(void) {
  PrecacheScriptSound("Weapon_Binoculars.Buzz");
  PrecacheScriptSound("Weapon_Binoculars.Signal");
