...
ConVar sk_plr_dmg_cguard("sk_plr_dmg_cguard", "200", FCVAR_REPLICATED);
...
public:
  ...
  void Explode(float magnitude, float damage) {
    ...
    RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), damage,
...
void CreateConcussiveBlast(const Vector &origin, const Vector &surfaceNormal,
                           CBaseEntity *pOwner, float magnitude, float damage) {
  QAngle angles;
...
DEFINE_FIELD(m_flChargeTime, FIELD_TIME), DEFINE_FIELD(m_bFired, FIELD_BOOLEAN),
...
void CWeaponCGuard::AlertTargets(void) {
  ...
  UTIL_TraceLine(vecSrc, impactPoint, MASK_SHOT, pPlayer, COLLISION_GROUP_NONE,
                 &tr);
...
void CWeaponCGuard::UpdateLasers(void) {
  ...
  for (int i = 0; i < 4; i++) {
    Vector ofs = start + (v_forward * offset[i][0]) + (v_right * offset[i][1]) +
                 (v_up * offset[i][2]);
    ...
    v_ang[PITCH] =
        UTIL_AngleMod(v_ang[PITCH] + ((1.0f - angleOffset) * 15.0f * vScale));
    v_ang[YAW] =
        UTIL_AngleMod(v_ang[YAW] + ((1.0f - angleOffset) * 15.0f * hScale));

    AngleVectors(v_ang, &v_dir);

    trace_t tr;
    UTIL_TraceLine(ofs, ofs + (v_dir * MAX_TRACE_LENGTH), MASK_SHOT, this,
                   COLLISION_GROUP_NONE, &tr);

    UTIL_Beam(ofs, tr.endpos, m_beamIndex, 0, 0, 2.0f, 0.1f, 2, 0, 1, 0, 255,
              255, 255, 32, 100);

    UTIL_Beam(ofs, tr.endpos, m_haloIndex, 0, 0, 2.0f, 0.1f, 4, 0, 1, 16, 255,
              255, 255, 8, 100);
  }
}
...
void CWeaponCGuard::PrimaryAttack(void) {
  ...
  // UTIL_ScreenShake( GetAbsOrigin(), 10.0f, 100.0f, 2.0f, 128, SHAKE_START,
  // false );
...
void CWeaponCGuard::DelayedFire(void) {
  ...
  if ((GetMaxClip1() != -1 && m_iClip1 == 0) ||
      (GetMaxClip1() == -1 && !pPlayer->GetAmmoCount(m_iPrimaryAmmoType)))
    return;
...
void CWeaponCGuard::AddViewKick(void) {
  ...
  pPlayer->ViewPunch(QAngle(random->RandomInt(-8, -12),
                            random->RandomInt(-2, 2),
                            random->RandomInt(-8, 8)));
}
