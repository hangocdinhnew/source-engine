...
// reloads. 			Secondary attack: cycles sniper scope through
// zoom levels.
//
...
void CWeaponSniperRifle::ItemPostFrame(void) {
  ...
  if (!((pPlayer->m_nButtons & IN_ATTACK) ||
    ...
    if (!HasAnyAmmo() && m_flNextPrimaryAttack < gpGlobals->curtime) {
      ...
      if (!(GetFlags() & ITEM_FLAG_NOAUTOSWITCHEMPTY) &&
      ...
      // has to after firing
      if (m_iClip1 == 0 && !(GetFlags() & ITEM_FLAG_NOAUTORELOAD) &&
          m_flNextPrimaryAttack < gpGlobals->curtime) {
        Reload();
        return;
      }
...
// because our 			ItemPostFrame is only called when the owner's
// next attack time has 			expired.
